<script>
	...
	import ReviewUserForm from '$lib/users/addUser/ReviewUserForm.svelte';
	...
	// list of state options passed to the address component
	// let stateData = [
	// 	{
	// 		title: "State 1",
	// 		uuid: "0"
	...
	function userFormEnhancement({ formElement, formData, action, cancel, submitter }){
		...
		// validate for important fields to be non empty
		let nonEmptyValidation  = [ formObject.userName , formObject.userRole, formObject.userId ]
		if(nonEmptyValidation.some((elem) => elem?.trim() === '')){
			validationError = "All fields marked with * must be filled"
			cancel()
			return 
		}
...
<div class="m-4 lg:m-12 text-primary">
	...
	<form method="POST" action="?/review" use:enhance={userFormEnhancement} enctype="multipart/form-data" >
		<!-- To be used while doing review add course -->
		<div>
			{#if !saved}
			<h2 class="mb-2 font-semibold">Add User</h2>
			<!-- First Row -->

			<div class="grid grid-cols-1 sm:grid-cols-2 sm:gap-x-20 gap-y-6 mb-2 lg:mb-12">
				<div class="flex flex-col gap-6">
					<div class="w-full">
						<InputField label={'Name *'} placeholder={'Name'} bind:value={formObject.userName} type="text" name="userName" required/>
					</div>
					<div class="w-full">
						<InputField label={'User ID *'} placeholder={'User ID'} bind:value={formObject.userId} type="text" name="userId" required/>
					</div>
					<div class="w-full">
						<InputField
							label={'Designation'}
				...
				<div class="w-full sm:col-span-1 flex flex-col items-center justify-center gap-4">
					...
					>
					<input type="file" name={'userImage'} bind:this={imageUploadInputRef} on:change={handleImageChange} class="hidden" />
				</div>

				<div class="sm:col-span-2 grid grid-cols-2 sm:gap-x-20">
					<div class="w-full ">
						<DropDown title="Role *" options={roleOptions} type={"Role"} on:handleDispatchFilterData={handleDropDown} bind:selectedItemName={formObject.userRole}/>
					</div>
				</div>
				...
				<Address
					...
					readonly={formObject.currentAddressSameAsPermanent}
					stateData =  {stateOptionList}
			...
			<ReviewUserForm saved={saved} userData={form?.formData}/>
			</div> 
			{/if}
