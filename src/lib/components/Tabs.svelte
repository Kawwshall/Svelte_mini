	...
	<nav class="flex flex-row flex-wrap">
		{#each tabs as tab}
			<button
				...
				class=" whitespace-nowrap cursor-pointer border-b-2 px-4 py-3 text-sm font-medium text-darkGray hover:border-orange-40 hover:text-gray-700 {tab?.text ===activeTabText
					? 'border-orange-100'
					: 'border-transparent'}"
			>
			<div class="flex items-center space-x-2">
				{#if tab?.icon}
					...
					/>
				{/if}
				<span class="text-xs">{tab?.textDispaly}</span>
			</div></button
			>
		{/each}
	</nav>
